### Annotations of the SNPs emanating from the GLM + LD analysis
### 
...
annotation_set <- foreach(i=1:dim(snps.dt.out)[1],
                          .combine = "rbind")%do%{ ## open DO loop ## MAYBE CHANGE TO DOPAR?
  
  print(i)
  subset_geno = genofile
  
  seqSetFilter(subset_geno, variant.id=snps.dt.out$variant.id[i])
      ...
      obj_to_rturn <- data.frame(chr = snps.dt.out$chr[i],
                                 pos = snps.dt.out$pos[i],
                                 tmp_annot_sep
                                  )
...
## create unique annotation list based on priority_scoring
annotation_set %>% 
  group_by(chr, pos) %>%
  slice_max(priority_scoring, with_ties = F) ->
  annotation_set_prioritized

write.table(annotation_set_prioritized,
            file = "./GLM_LD_outliers_annotation_priorized.txt", 
            append = FALSE, 
            quote = FALSE, 
            sep = "\t",
            eol = "\n", 
...
### Analyze annotations
annotation_set %>% 
  filter(Annotation %in% c("missense_variant") ) %>%
  group_by(chr, pos, Annotation, Gene_Name, HGVS.c,	HGVS.p) %>%
  summarise(N= n(),
            Mean_priority = mean(priority_scoring) ) -> missense_variants_summary

write.table(missense_variants_summary,
            file = "./missense_variants.annot.txt", 
            append = FALSE, 
            quote = FALSE, 
            sep = "\t",
            eol = "\n", 
...
annotation_set %>% 
  ...
  summarise(N= n()) %>%
  group_by(pos) %>%
  slice_head() -> missense_variants_nonRedundant

write.table(missense_variants_nonRedundant,
            file = "./missense_variants_nonRedundant.txt", 
            append = FALSE, 
            quote = FALSE, 
            sep = "\t",
            eol = "\n", 
