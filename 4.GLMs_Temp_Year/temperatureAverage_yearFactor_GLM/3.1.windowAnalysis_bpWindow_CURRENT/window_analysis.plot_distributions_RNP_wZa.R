...
#### version that uses counts
  ...
  rnp.ag <- win.out[nSNPs>50,list(y=.N), list(mod, chr.x, inv=invName!="none", locality, perm, x=rnp.pr.bin)]
  wza.ag <- win.out[nSNPs>50,list(y=.N), list(mod, chr.x, inv=invName!="none", locality, perm, x=wZa.bin)]
