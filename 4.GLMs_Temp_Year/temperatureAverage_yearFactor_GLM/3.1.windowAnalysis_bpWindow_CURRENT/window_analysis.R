...
### run windows
  ...
  win.out <- foreach(win.i=c(1:dim(wins)[1]), .errorhandling="remove")%dopar%{
    ...
    win.tmp[!is.na(rnp.clean),
              list(rnp.pr=c(mean(rnp.clean<=0.01),mean(rnp.clean<=0.05)),
                  pr=c(0.01, .05),
                  rbinom.p=c(binom.test(sum(rnp.clean<=0.01), length(rnp.clean), .01)$p.value,
                              binom.test(sum(rnp.clean<=0.05), length(rnp.clean), .05)$p.value),
...
### save
  save(win.out, file=paste("/scratch/aob2x/dest_glm_morePerms_nested_qb/windowAnalysis/WZA_window.dt.",
