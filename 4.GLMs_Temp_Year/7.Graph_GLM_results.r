...
win.out.final.real.perm %>%
  ...
  summarize(quant_h=quantile(rnp.binom.p, 0.01),
            quant_l=quantile(rnp.binom.p, 0.99)
              ) ->
...
##  win.out_quant.summ.dcast
  ...
  geom_point(data = filter(win.out_quant.summ.processed, rnp.binom.p <  quant_h), 
...
ggsave(plot_perm_real_final, file = "plot_perm_real_final.pdf")
