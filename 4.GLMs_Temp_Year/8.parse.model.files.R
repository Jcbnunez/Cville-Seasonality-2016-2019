...
###
out_file = "/scratch/yey2sn/Overwintering_ms/4.1.NewTempModels/GLM_new_mods_data/"
...
fl <- paste0("/project/berglandlab/alan/dest_glm_final_nested_qb_alternatives_AIC/dest_glm_final_nested_qb_alternatives_AIC/job", c(1:1000), ".Rdata")
#use a for loop to save file for each permutation

#foreach(g = c(0:100))%do% { 

out = foreach(f = c(1:950), 
              .combine = "rbind") %dopar% {
                ...
                left_join(glm.out, sets) %>%
                  left_join(snps.dt[,c("variant.id","chr", "pos", "nAlleles")]) %>% 
                  mutate(job = f) ->
                  glm.out.annot
