...
### load glm.out
fl <- list.files("/scratch/yey2sn/Overwintering_ms/4.1.NewTempModels/GLM_window_analysis", full.names=T)

all.mod.out = foreach(i=1:101, .combine = "rbind")%do%{
  message(i)
  load(fl[i])
  win.out
}
