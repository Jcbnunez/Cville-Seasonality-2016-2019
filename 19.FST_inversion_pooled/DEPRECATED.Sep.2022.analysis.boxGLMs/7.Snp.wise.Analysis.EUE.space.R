...
selected_pops = c("UA_Cho", "UA_Dro", "UA_Kan", "UA_Kha", "UA_Kyi",  "UA_od", "UA_Ode", "UA_Pir", "UA_Pyr", "UA_Uma", "UA_Var", "UA_Che", "UA_Yal", "RU_Ale", "RU_tul", "RU_val", "RU_Val")
...
####
dat.in.qtile = filter(snp.wise.tmp.f.df.ag, type == "glm")
controls.qtile = filter(snp.wise.tmp.f.df.ag, type == "control")
...
quantiles = foreach(i=1:dim(dat.in.qtile)[1], .combine = "rbind")%do%{
  ...
  message(paste(i , #dy.set
                SNP_id.set, fst.glm,  sep = " | "))
  
  controls.qtile %>%
    filter(#dy == dy.set & 
             matched_to == SNP_id.set ) ->
    fst.dist.tmp
...
ggplot() +
  geom_boxplot(data = filter(snp.wise.tmp.f.df.ag, type == "control"
  ),
  aes(x=as.factor(matched_to),
      #color = as.factor(matched_to),
      y=median.fst),
  outlier.shape = NA, width = 0.6) +
  geom_point(data = left_join(filter(snp.wise.tmp.f.df.ag, type == "glm"), quantiles),
             aes(x=as.factor(matched_to),
                 y=median.fst),
             size = 5,
             color = "red") +
  geom_text(data = left_join(filter(snp.wise.tmp.f.df.ag, type == "glm"), quantiles),
             aes(x=as.factor(matched_to),
                 y=median.fst, #+ 0.0015,
  ...
  ylim(-0.05, 0.05) +
  theme_bw() +
  ggtitle("EUE: Ukriane and Russia") +
