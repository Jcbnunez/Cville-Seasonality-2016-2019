...
### import
  ...
  save(gwas.win.o, file="~/all_gwas_glm_win2.Rdata")


  gwas.win.o[,grm:=!grepl("nogrms", gwas.pheno)]
  gwas.win.o[,nSNPs:=TT+TF+FT+FF]
  gwas.win.o.ag <- gwas.win.o[nSNPs>250, list(n=sum(pa<.05 & or>1)), list(chr, start, end, grm, glm.perm.n, inv, win.i)]
  gwas.win.o.ag.ag <- gwas.win.o.ag[,list(n=mean(n), sd=sd(n)), list(chr, start, end, grm, win.i, perm=I(0!=glm.perm.n), inv)]

  save(gwas.win.o.ag, gwas.win.o.ag.ag, file="~/all_gwas_glm_win3.Rdata")
...
### noodling
  ...
  setkey(gwas.win.o, chr)
  gwas.win.o[J("2L"), list(.N), list(glm.perm.n, inv)]

  table(gwas.win.o[J("2L")]$pa<.05, gwas.win.o[J("2L")]$glm.perm.n, gwas.win.o[J("2L")]$inv)

  table(gwas.win.o[chr=="2L"][glm.perm.n==0][pa<.05][inv==T]$gwas.pheno)

### copy to load and load
### scp aob2x@rivanna.hpc.virginia.edu:~/all_gwas_glm_win3.Rdata ~/.
  ...
  gwas.win.o.ag <- gwas.win.o[,list(n=sum(pa<.05 & or>1), terms=paste(gwas.)),
                                    list(chr, start, end, grm, glm.perm.n, inv, win.i)]
  gwas.win.o.ag.ag <- gwas.win.o.ag[,list(n=median(n), sd=sd(n)),
                                    list(chr, start, end, grm, perm=I(0!=glm.perm.n), inv)]
