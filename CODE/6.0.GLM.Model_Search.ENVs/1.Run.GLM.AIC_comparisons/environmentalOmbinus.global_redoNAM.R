...
### also  clean up based on other flags from Kapun et al
  ...
  samps <- samps[!locality%in%c("GS_Des", "GS_Sai")]
  samps[locality%in%c("IL_ur", "KA_to", "MI_bh", "ON_su", "WI_cp"), cluster:="2.North_America_Mid"]
  table(samps$cluster)

  samps <- samps[cluster%in%c("2.North_America_E", "2.North_America_Mid")]
  table(samps$sampleId, samps$mod, samps$cluster)
  ...
  ### sequential GLM
    ...
    glm.out <- foreach(i=snp.dt[job==jobId]$id)%dopar%{
      ...
      ### for each population cluster
        o <- foreach(cluster.i=na.omit(unique(data$cluster)))%do%{
          # cluster.i <- "5.Cville"
          ...
          o <- foreach(perm.i=0:100, .combine="rbind")%dopar%{
            ...
            if(perm.i==0) {
              ...
              ord.tmp <- foreach(mod.i=unique(samps$mod), .combine="rbind")%do%{
                set.seed(perm.i)
                samps[mod==mod.i & cluster==cluster.i,list(year=year, mod=mod.i,
                                      temp.ave,
                                      temp.var,
                                      temp.max,
                                      temp.min,
...
### split and save
  ...
  save(glm.out, file=paste(mainDir, "/job", jobId, ".redoNAM.Rdata", sep=""))
