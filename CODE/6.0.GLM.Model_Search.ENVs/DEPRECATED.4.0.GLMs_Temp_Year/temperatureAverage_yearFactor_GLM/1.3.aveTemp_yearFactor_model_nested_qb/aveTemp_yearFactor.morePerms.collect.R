...
### collect results
  perm.sets <- list.files("/scratch/aob2x/dest_glm_final_nested_qb/rawGLM", ".csv", full.name=T)
...
### assign ranks: (1) all tested sites; (2) removing rep elements & cM/Mb==0
  ...
  lrt.rank.clean <- glm.out[N==0 & cm_mb>0 & !is.na(cm_mb) & chr!="X",
                            list(rp.clean=rank(p.lrt), rChi.clean=rank(-chisq), l.clean=length(p.lrt), id),
                            list(mod, perm)]


  lrt.rank.all <- glm.out[, list(rp.all=rank(p.lrt), rChi.all=rank(-chisq), l.all=length(p.lrt),
                                    id),  list(mod, perm)]
  ...
  glm.out[,rnp.clean:=rp.clean/(l.clean+1)]
  glm.out[,rnp.all:=rp.all/(l.all+1)]

  glm.out[,rnChi.clean:=rChi.clean/(l.clean+1)]
  glm.out[,rnChi.all:=rChi.all/(l.all+1)]

#### FDR
  q.clean <- glm.out[N==0 & cm_mb>0 & !is.na(cm_mb) & chr!="X", list(q.clean=p.adjust(p.lrt, "fdr"),
                                    id),  list(mod, perm)]
...
#  save(glm.out, file=paste("/scratch/aob2x/summarized_dest_glm/glm.out.", glm.out[1]$locality, "_", glm.out[1]$perm, ".Rdata", sep=""))
  save(glm.out, file=paste("/scratch/aob2x/dest_glm_final_nested_qb/processedGLM/glm.out.", glm.out[1]$locality, "_", glm.out[1]$perm, ".Rdata", sep=""))
...
# table(glm.out[mod=="aveTemp+year_factor"]$rnp.clean<.001, 
#       glm.out[mod=="aveTemp+year_factor"]$chr, 
#       glm.out[mod=="aveTemp+year_factor"]$invName!="none")
# 
# 
# table(glm.out[mod=="aveTemp+year_factor"]$rnChi.clean<.001, 
#       glm.out[mod=="aveTemp+year_factor"]$chr, 
#       glm.out[mod=="aveTemp+year_factor"]$invName!="none")
# 
# 
# table(glm.out$q.clean<.05, glm.out$chr, glm.out$mod)
# 
# glm.out[mod=="aveTemp+year_factor"][q.clean<.05][chr=="2L"]
# 
#### temperature test
