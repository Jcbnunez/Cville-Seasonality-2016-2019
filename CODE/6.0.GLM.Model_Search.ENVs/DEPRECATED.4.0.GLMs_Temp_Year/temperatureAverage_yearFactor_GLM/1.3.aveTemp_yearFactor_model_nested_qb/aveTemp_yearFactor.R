        ...
        o <- foreach(perm.i=0:100, .combine="rbind")%dopar%{
          ...
          ### foreach UseSet that passes filter
            ...
            mods.out <- foreach(useSet.i=usePops[value==T]$variable, .combine="rbind")%do%{
              # useSet.i <- "VA_ch"
              t0 <- glm(af_nEff~1, data=data.use[locality==useSet.i], family=quasibinomial(), weights=data.use[locality==useSet.i]$nEff)
              ...
              # aveTemp.aov <- anova(t0, t1.aveTemp, test="Chisq")
               ...
               t1.aveTemp <- glm(af_nEff~ as.factor(year) + (aveTemp), data=data.use[locality==useSet.i], family=quasibinomial(), weights=data.use[locality==useSet.i]$nEff)
               t1.aveTemp.sum <- (summary(t1.aveTemp))
               aveTemp.aov <- anova(t1.year, t1.aveTemp, test="Chisq")


              rbind(
                data.table(variant.id=i, locality=useSet.i, perm=perm.i,
                          mod="year_factor", year=paste(tstrsplit(row.names(coef(t1.year.sum))[-1], ")")[[2]], collapse=";"), delta=30,
                           ...
                           chisq=year.aov$Deviance[2],p.lrt=year.aov$Pr[2],
                           df=year.aov$Df[2], nSamps=sum(!is.na(data.use[locality==useSet.i]$nEff))),

                data.table(variant.id=i, locality=useSet.i, perm=perm.i,
                          mod="aveTemp+year_factor", year=paste(tstrsplit(row.names(coef(t1.aveTemp.sum))[-1], ")")[[2]], collapse=";"), delta=30,
                           p=paste(coef(t1.aveTemp.sum)[-1,4], collapse=";"), b=paste(coef(t1.aveTemp.sum)[-1,1], collapse=";"), a=coef(t1.aveTemp.sum)[1,1],
                           chisq=aveTemp.aov$Deviance[2], p.lrt=aveTemp.aov$Pr[2],
...
### split and save
  ...
  mainDir="/scratch/aob2x/dest_glm_final_nested_qb"
  ...
  foreach(locale.i=unique(samps$locality))%do%{
    ...
    write.csv(tmp,
        file=paste(mainDir, "/", subDir, "/", locale.i, ";allPerm;aveTemp_yearFactor_nested_qb;", jobId, ".csv", sep=""), quote=F, row.names=F)
  }
