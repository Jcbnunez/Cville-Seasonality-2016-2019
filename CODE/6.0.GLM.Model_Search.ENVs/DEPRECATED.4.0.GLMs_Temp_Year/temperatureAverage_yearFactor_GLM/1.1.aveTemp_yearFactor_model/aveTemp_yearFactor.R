...
################
  ...
  ### sequential GLM
    glm.out <- foreach(i=snp.dt[job==jobId]$id)%dopar%{
      ...
      ### permutations
        o <- foreach(perm.i=11:100, .combine="rbind")%dopar%{
          if(perm.i==0) {
...
### split and save
  ...
  mainDir="/scratch/aob2x/dest_glm_morePerms"
  ...
  foreach(locale.i=unique(samps$locality))%do%{
    ...
    write.csv(tmp,
        file=paste(mainDir, "/", subDir, "/", locale.i, ";allPerm;aveTemp_yearFactor;", jobId, ".csv", sep=""), quote=F, row.names=F)
  }
