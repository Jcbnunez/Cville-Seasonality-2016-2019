...
### load glm.out
  ...
  fl <- list.files("/scratch/aob2x/summarized_dest_glm_nested/windowAnalysis/", ".Rdata", full.names=T)

  win.out <- foreach(fl.i=fl, .combine="rbind")%dopar%{
    fl.i <- "/scratch/aob2x/summarized_dest_glm_nested/windowAnalysis//WZA_window.dt.VA_ch_0.Rdata"
    message(fl.i)
    load(fl.i)
    win.out
  }
...
### save
  ...
  save(win.out, file="/scratch/aob2x/window_analysis_output.nested.Rdata")
