...
################
  ...
  ### open GDS for common SNPs (PoolSNP)
   ...
   genofile <- seqOpen("/Users/alanbergland/dest.all.PoolSNP.001.50.10Mar2021.ann.gds")
  ...
  ### extract allele frequencies function
    getData <- function(chr="2L", start=14617051, end=14617051, samplesUse=samps$sampleId) {
      # chr="2L"; start=9504688; end=9504688; samplesUse=samps$sampleId
        ...
        o <- foreach(perm.i=0:100, .combine="rbind")%dopar%{
          ...
          ### foreach UseSet that passes filter
            ...
            mods.out <- foreach(useSet.i=usePops[value==T]$variable, .combine="rbind")%do%{
              # useSet.i <- "VA_ch"
              t0 <- glm(af_nEff~1, data=, family=quasibinomial(), weights=data.use[locality==useSet.i]$nEff)

              tmp <- data.use[locality==useSet.i]

              t1.year <- glm(af_nEff~as.factor(year), data=tmp, family=quasibinomial(), weights=tmp$nEff)
              t1.year.sum <- (summary(t1.year))
              year.aov <- anova(t0, t1.year, test="Chisq")

              t1.aveTemp <- glm(af_nEff~ as.factor(year) + (aveTemp), data=tmp, family=quasibinomial(), weights=tmp$nEff)
              t1.aveTemp.sum <- (summary(t1.aveTemp))
              aveTemp.aov <- anova(t1.year, t1.aveTemp, test="Chisq")

              tmp[,fit:=t1.aveTemp$fitted.value]
...
### split and save
  ...
  mainDir="/scratch/aob2x/dest_glm_morePerms_nested_qb"
