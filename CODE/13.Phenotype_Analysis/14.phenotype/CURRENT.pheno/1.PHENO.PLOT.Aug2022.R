...
### target phenotypes
  target <- fread("./hotandcold.phenos.txt")
...
### load GLM
  glm.all <- get(load("/project/berglandlab/alan/environmental_ombibus_global/temp.max;2;5.Cville/temp.max;2;5.Cville.glmRNP.Rdata"))
  
  glm.out = glm.all %>%
            filter(perm == 0)
  
  best <- glm.out[variable=="temp.max"][chr=="2L"][rnp<.05][pos>=(5192177-10000)][pos<=(5192177+10000)]
...
### open DGRP genotype data
  ...
  setkey(best, chr, pos)
  snp.dt[J(best)]
  seqSetFilter(genofile, variant.id=snp.dt[J(best)]$variant.id)
...
### merge
  pl <- merge(pl, genos, by="ral_id", allow.cartesian=T)
...
### loadings plot
  ...
  ggsave( viz + p1 + p2, file="./mega3.pdf", h=12, w=12)
