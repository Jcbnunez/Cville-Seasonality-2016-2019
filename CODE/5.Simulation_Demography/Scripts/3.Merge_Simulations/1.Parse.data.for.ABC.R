...
nMax=as.numeric(args[1])
nMin=as.numeric(args[2])

base <- "/project/berglandlab/connor/bottleneck"
#metric <- "pca";nMax=28000;nMin=25000

temp.o2 <- foreach(metric = c("afvar", "pca", "euc", "fst"), 
                   .combine = "rbind", .errorhandling  = "remove") %do% { # opening o2
                     
                     ### global step                   
                     root <- paste(base, paste("data3", metric, sep = "."), sep = "/")
                     file.name <- paste("freq.output.new", metric,nMax, nMin, "*", "*" , "csv", sep = ".")
                     file.tmp <- system( paste("ls ", root,  " | grep ", "'" , file.name, "'" , sep = "") , intern = T )
                     
                     ### next read all files anc concatenate
                     if(metric == "afvar"){ ## if afar
                       temp.o <- foreach(i=1:length(file.tmp), .combine = "rbind", .errorhandling  = "remove")%do%{
                         
                         tmp.f <- fread( paste(root, file.tmp[i], sep = "/")  )  
                       } #closing o2
                       
                       colnames(temp.o) <- c("Mean.var","Median.var","IQR95.var",
                                             "IQR05.var","replicate","nMax","nMin","model")
                       # Average across replicates
                       dt2.var <- data.table(temp.o %>%
                     ...
                     if(metric == "fst"){ ## if afar
                       temp.o <- foreach(i=1:length(file.tmp), .combine = "rbind", .errorhandling  = "remove")%do%{
                         
                         tmp.f <- fread( paste(root, file.tmp[i], sep = "/")  )  
                       } #closing o2
                       
                       colnames(temp.o) <- c("samp1","samp2","FST","iteration","nMax","nMin",
                                             "replicate","seed","YearS1","sim.gen1","YearS2","sim.gen2","Year")
                       
                       # Average across replicates
                       dt2 <- data.table(temp.o %>%
                     ...
                     if(metric == "euc"){
                       ...
                       colnames(temp.o) <- c("year.diff","Mean.euc","Median.euc",
                                             "IQR05.euc","IQR95.euc","data",
                                             "replicate","nMax","nMin","model")
                       # Average across replicates
                       dt2.euc <- data.table(temp.o %>%
...
system("mkdir Parsed_sims")
save(temp.o2, file = paste("/scratch/csm6hg/bottleneck/Parsed_sims/", paste("Sim", nMax, nMin, sep = "."), ".Rdata" , sep = "") )
