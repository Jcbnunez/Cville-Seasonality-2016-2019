...
# sbatch --array=1-7 ~/Overwintering_18_19/temperatureAverage_yearFactor_GLM/1.2.aveTemp_yearFactor_model_nested/aveTemp_yearFactor.morePerms.collect_part1.sh
# sacct -j 28814975
...
dir=$( ls -d /scratch/aob2x/dest_glm_morePerms_nested/* | sed "${SLURM_ARRAY_TASK_ID}q;d" )
echo $dir

split_fun () {
  fn=${1} # fn="/scratch/aob2x/dest_glm_morePerms/DE_Bro/DE_Bro;allPerm;aveTemp_yearFactor;100.csv"
  echo ${fn}
  cd /scratch/aob2x/dest_glm_morePerms_nested
  cat ${fn} |
  awk -F ',' '{
    print $0 >> $2"_"$3".csv"
  }'
