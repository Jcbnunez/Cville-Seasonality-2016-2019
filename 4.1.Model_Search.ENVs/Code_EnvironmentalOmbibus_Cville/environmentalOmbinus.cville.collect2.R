...
### load
  load(file="/project/berglandlab/alan/environmental_ombibus/mod_var.Rdata")
...
### split output and get summaries
  ...
  glm.out <- foreach(fl.i=fl, .errorhandling="remove")%dopar%{
    ...
    tmp <- glm.out[J(data.table(variable =strsplit(mvi, "_")[[1]][1],
                                mod  =as.numeric(strsplit(mvi, "_")[[1]][2])))]
