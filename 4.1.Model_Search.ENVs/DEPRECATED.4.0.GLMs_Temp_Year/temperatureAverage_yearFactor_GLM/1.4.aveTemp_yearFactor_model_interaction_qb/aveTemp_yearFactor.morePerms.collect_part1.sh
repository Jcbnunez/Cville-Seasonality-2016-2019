...
# sbatch --array=8 ~/Overwintering_18_19/temperatureAverage_yearFactor_GLM/1.3.aveTemp_yearFactor_model_nested_qb/aveTemp_yearFactor.morePerms.collect_part1.sh
# sacct -j 28887924
# sacct -j 28887933
# cat /scratch/aob2x/lasso_dest/slurmOut/cm_lasso_28814946_5.err
### SLURM_ARRAY_TASK_ID=1

# SLURM_ARRAY_TASK_ID=1

dir=$( ls -d /scratch/aob2x/dest_glm_morePerms_nested_qb/* | sed "${SLURM_ARRAY_TASK_ID}q;d" )
echo $dir

split_fun () {
  fn=${1} # fn="/scratch/aob2x/dest_glm_morePerms/DE_Bro/DE_Bro;allPerm;aveTemp_yearFactor;100.csv"
  echo ${fn}
  cd /scratch/aob2x/dest_glm_morePerms_nested_qb/rawGLM/
  cat ${fn} |
  awk -F ',' '{
    print $0 >> $2"_"$3".csv"
  }'
