...
### quick test
  ...
  o <- foreach(set.i=c("aveTemp+year_factor", "year_factor"), .combine="rbind", .errorhandling="remove")%do%{
    foreach(mod.i=c("cline"), .combine="rbind", .errorhandling="remove")%do%{
      #mod.i <- "cline"; set.i<-"aveTemp+year_factor"
      tmp <- m[J(data.table(set=set.i, mod=mod.i, key="mod,set"))][!is.na(rnp.clean) & !is.na(clinal.p)]

      tmp[,rnp.clean:=rank(p.lrt)/(length(p.lrt+1))]
      tmp[,rnp.machado:=rank(clinal.p)/(length(clinal.p+1))]
      tmp[,inv:=invName!="none"]

      m1.beta <- tmp[,list(thermal.beta=tstrsplit(b, ";")%>%last%>%as.numeric, seas.beta=clinal.coef), list(id)]

      tmp <- merge(tmp, m1.beta, by="id")
      ...
      o <- foreach(chr.i=unique(tmp$chr), .combine="rbind", .errorhandling="remove")%do%{
        foreach(inv.i=c(T,F), .combine="rbind", .errorhandling="remove")%do%{
          foreach(thr.i=thrs, .combine="rbind", .errorhandling="remove")%dopar%{
              ...
              st.T <- sum(sign( tmp[chr==chr.i][inv==inv.i][rnp.clean<thr.i & rnp.machado<thr.i]$thermal.beta) ==
                           sign(tmp[chr==chr.i][inv==inv.i][rnp.clean<thr.i & rnp.machado<thr.i]$seas.beta))
              st.F <- sum(sign( tmp[chr==chr.i][inv==inv.i][rnp.clean<thr.i & rnp.machado<thr.i]$thermal.beta) !=
                           sign(tmp[chr==chr.i][inv==inv.i][rnp.clean<thr.i & rnp.machado<thr.i]$seas.beta))
